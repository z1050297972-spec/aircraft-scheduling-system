...
const CaseCard: React.FC<Props> = ({ testCase, idx, onOpenEditModal, onDelete, onLoad }) => {
  return (
    ...
    >
      ...
      <div className={`h-1 w-full bg-gradient-to-r ${testCase.predictedOutcome.score >= 90 ? 'from-emerald-400 to-emerald-600' :
      ...
      <div className="p-6 pb-4 border-b border-slate-50 relative">
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1 mr-2">
            <h3 className="font-bold text-slate-800 group-hover:text-blue-600 transition-colors text-lg break-words" title={testCase.scenarioName}>
              {testCase.scenarioName}
            </h3>
          </div>
          <div className={`flex flex-col items-center justify-center w-10 h-10 rounded-lg shrink-0 ${testCase.predictedOutcome.grade === '优秀' ? 'bg-emerald-50 text-emerald-700' :
